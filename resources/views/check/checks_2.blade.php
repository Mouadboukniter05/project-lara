...
{{-- 
        ...
        <td>
            ...
            @if ($check->priority == 1 && $check->completed == 0 )
            {{-- <a href="{{ route('PDF.Print', ['id' => $check->id]) }}" class="btn btn-info"><span class="glyphicon glyphicon-print" aria-hidden="true"></span></a> --}}
            <button onclick="printJS('{{ route('PDF.Print', ['id' => $check->id]) }}')" class="btn btn-info"><span class="glyphicon glyphicon-print" aria-hidden="true"></span></button>
            @endif
