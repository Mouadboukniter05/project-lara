...
<form action="{{ route('projet.update', [ 'id' => $projet->id ] ) }}" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
    ...
    <div class="col-md-8">
    	...
    	<div class="form-group">
			...
			<input type="text" class="form-control"  name="projet_title" value="{{ $projet->projet_title }}">
		</div>
    	...
    	<div class="form-group">
			...
			<textarea class="form-control my-editor" rows="5" id="projet" name="projet">{{ $projet->projet }}</textarea>
		</div>
		...
		<label>Files</label>
		<ul class="fileslist">
           	@foreach( $projetfiles as $file) 
			    <li>{{ $file->filename }} <span>&nbsp;&nbsp;</span> <a class="btn btn-danger" href="{{ route('projet.deletefile', [ 'id' => $file->id]) }}">
              ...
              <select name="customer_id" id="customer_id" class="form-control">
                    @foreach( $customers as $customer)
                        <option value="{{ $customer->id }}" 
                          @if( $projet->customer->id == $customer->id )
                                selected
                          @endif
        ...
        <div class="form-group">
            <label>Edit Due Date <span class="glyphicon glyphicon-calendar" aria-hidden="true"></span></label>
                ...
                <div class='input-group date' id='datetimepicker1'>
					<input type='text' class="form-control" name="duedate" value="{{ $projet->duedate }}">
