...
<form action="{{ route('report.update', [ 'id' => $report->id ] ) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="col-md-8">
    	...
    	<div class="form-group">
			...
			<input type="text" class="form-control"  name="report_title" value="{{ $report->raport_title }}">
		</div>

    	<div class="form-group">
    		<label>{{ __('messages.Report') }}</label>
			<textarea class="form-control my-editor" rows="5" id="report" name="report">{{ $report->raport }}</textarea>
		</div>
	...
	<div class="col-md-4">
        ...
        <div class="form-group">
            <label>{{ __('messages.Created_At') }} <span class="glyphicon glyphicon-calendar" aria-hidden="true"></span></label>
     
                <div class='input-group date' id='datetimepicker1'>
					<input type='text' class="form-control" name="duedate" value="{{ $report->duedate }}">
